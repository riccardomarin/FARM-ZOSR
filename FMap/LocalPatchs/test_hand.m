%%
function [smpl_idx, Dato_idx] = test_hand(name,land, plotflag)
...
% extract heads
M = extract_patch(M_full, calc_dist_map(M_full, landmarks2(land)) < dist_thresh);
N = extract_patch(N_full, calc_dist_map(N_full, landmarks1(land)) < dist_thresh);
M_headtip = find(landmarks2(land)==M.fullshape_idx);
N_headtip = find(landmarks1(land)==N.fullshape_idx);
...
% figure
% subplot(121), plot_mesh(M)
% subplot(122), plot_mesh(N)
...
parfor iter=1:n_angles
    ...
    N_deformable = align_icp_cpd_hand(M, N_deformable, 3);
